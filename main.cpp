...
void store(int id)
{
    ...
    if (map.find(id) != map.end())
    {
        ...
        case 0:
            ...
            break;
        case 1:
            str = " Customer with ID " + to_string(id) + " subscribed to bundle " + to_string(bundleNum) ;
            Log(str);
...
void performOperation()
{
    ...
    {
        string str = " Customer with ID " + to_string(ID) + " found!";
        Log(str);
        cout << "Valid ID." << endl;
...
void insertmap(unordered_map<long, int> &map, long ID, int numofOP)
{
    map[ID] = numofOP;
}
